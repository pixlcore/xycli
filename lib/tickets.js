...
module.exports = {
	...
	async cmd_ticket() {
		...
		switch (cmd) {
			...
			case 'download': await this.cmd_download_ticket_file(); break;
	...
	async cmd_get_ticket() {
		...
		if (ticket.events && ticket.events.length) this.printTicketEvents(ticket.events || []);
		if (ticket.files && ticket.files.length) this.printTicketFiles(ticket.files || []);
		...
		this.printSuggestedCommands({
			...
			"Download a file": ticket.files && ticket.files.length ? `xy ticket download ${ticket.num} ${ticket.files[0].id}` : '',
			...
			"Delete Ticket": `xy ticket delete ${ticket.num}`
		});
	},
	...
	async cmd_download_ticket_file() {
		// Ticket attachments are served from their storage paths through the same
		// authenticated binary endpoint used by Bucket files.  Always load the
		// Ticket because even exact Ticket IDs do not contain file metadata.
		var ticket_selector = this.consumeTicketSelector(true);
		var file_selector = this.args.other.shift() || this.args.filename || this.args.file;
		if (!file_selector) return this.dieUsage('ticket download');
		...
		if (Tools.numKeys(this.args)) return this.die("Unsupported Ticket download option: --" + Tools.firstKey(this.args));

		var ticket = await this.fetchTicket(ticket_selector);
		var file = this.findTicketFile(ticket.files, file_selector);
		if (!file) return this.die("Could not find Ticket file from exact filename or File ID: " + file_selector);
		...
		if (this.dry || this.verbose) {
			println("\n " + this.color('theme').bold('TICKET FILE DOWNLOAD:'));
			this.jsonOutput({
				ticket: ticket.id,
				number: ticket.num,
				file: file.filename,
		...
		if (this.dry) {
			this.toast('⚠️', 'orange', bold("DRY RUN: ") + "Exiting without downloading file.");
			return;
		}

		println("\n " + this.color('theme').bold('TICKET FILE: ' + file.filename));
		println(" " + gray("Downloading to file: ") + dest_file);
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Downloading Ticket file...') });
		try {
			...
			// belongs to this failed request and is safe to remove.
			if (fs.existsSync(dest_file)) fs.unlinkSync(dest_file);
			return this.die(err);
		}
		cli.progress.end();
		this.toast('✅', 'green', "Successfully downloaded Ticket file: " + file.filename);
	},
	...
	findTicketFile(files, selector) {
		// File IDs are unique.  Filenames are convenient, but Ticket uploads may
		// contain duplicates, so require the File ID when a name is ambiguous.
		selector = String(selector);
		var id_match = Tools.findObject(files || [], { id: selector });
		if (id_match) return id_match;

		var name_matches = (files || []).filter( file => file.filename == selector );
		if (name_matches.length > 1) this.die("Multiple Ticket files have this filename. Please use a File ID: " + selector);
		return name_matches[0] || null;
	},
