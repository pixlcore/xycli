...
module.exports = {
	...
	async cmd_get_server() {
		...
		if (pid !== undefined) {
			...
			this.printServerProcessDetail(server, live_data, Number(pid), detail);
			return;
		}
		...
		if (show_monitors && !live_data.dummy) {
			requests.push( this.api.getLatestMonitorData({ server: server.id, sys: 'hourly', limit: 60 }).then( result => {
				if (result.err) throw result.err;
				// The current interval may be represented by a sparse placeholder
				// while xySat is still assembling it.  Charts only consume complete rows.
				monitor_rows = (result.data.rows || []).filter( row => row && row.date && row.totals );
			}) );
		}
	...
	printServerQuickmonCharts(server, rows, heading) {
		...
		println('\n ' + this.color('theme').bold(heading || 'QUICK LOOK - LAST MINUTE'));
	...
	printServerMonitorCharts(server, rows, online, heading) {
		var monitors = this.getVisibleServerMonitors(server);
		if (!monitors.length) return;
		var min_epoch = rows.length ? rows[rows.length - 1].date - 3600 : 0;
		rows = rows.filter( row => !min_epoch || (row.date >= min_epoch) );
		
		println('\n ' + this.color('theme').bold(heading || ('SERVER MONITORS - ' + (online ? 'LAST HOUR' : 'LAST KNOWN STATE'))));
		monitors.forEach( (def, idx) => {
			var data = this.getServerMonitorChartData(rows, def);
			println('\n' + cli.chart({
				title: def.title,
	...
	printServerProcessDetail(server, data, pid, detail) {
		...
		this.printBoxList({
			...
			rows: [
				['Server', this.getNiceServer(server)],
				['Command', command],
	...
	parseServerHistoryRange(value, label) {
		...
		if (!value.match(/^\d{4}(?:\/\d{1,2}){0,3}$/)) {
			this.die(label + ' history date must use YYYY, YYYY/MM, YYYY/MM/DD, or YYYY/MM/DD/HH format.');
		}
		...
		if ((year < 1970) || (year > 9999) || (month < 1) || (month > 12) || (day < 1) || (day > 31) || (hour < 0) || (hour > 23)) {
			this.die('Invalid ' + label + ' history date: ' + value);
		}
