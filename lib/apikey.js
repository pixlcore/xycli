...
module.exports = {
	...
	async cmd_get_api_keys() {
		...
		this.printPaginatedBoxTable({
			...
			header: ['Key ID', 'App Title', 'Partial Key', 'Status', 'Last Used'],
			rows: rows,
			list: { length: total },
			offset: this.offset,
			limit: this.limit
	...
	async cmd_get_api_key() {
		...
		// App descriptions may contain multiple lines, just like user notes.
		if (api_key.description) {
			this.printUserNotes('APP DESCRIPTION', api_key.description);
		}
		...
		this.printSuggestedCommands({
			...
			"Delete API Key": `xy key delete ${api_key.id}`,
	...
	async cmd_update_api_key() {
		...
		if (!await this.confirmSyncUpdate('api_key', api_key.id, 'API key')) return;
