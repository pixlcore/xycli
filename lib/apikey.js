...
module.exports = {
	...
	async cmd_key() {
		// Singular router, e.g. `xy key add` or `xy key update KEY_ID`.
		...
		switch (cmd) {
			...
			case 'update':
			case 'edit': await this.cmd_update_api_key(); break;
			case 'delete':
			case 'remove': await this.cmd_delete_api_key(); break;
	...
	async cmd_get_api_keys() {
		...
		this.printSuggestedCommands({
			...
			"Add API Key": "xy key add --title \"My App\" --privileges.admin",
	...
	async cmd_create_api_key() {
		...
		if (!params.title || !String(params.title).trim()) return this.dieUsage('key add');
	...
	async cmd_update_api_key() {
		...
		delete this.args.id;
		delete this.args.other;
		if (!Tools.numKeys(this.args)) return this.dieUsage('key update');
	...
	async cmd_delete_api_key() {
		...
		var api_key = Tools.findObject(this.api_keys, { id: id });
		if (!api_key) return this.die("Could not find API Key from ID: " + id);
		if (!this.args.confirm) return this.die("Please confirm the delete by adding '--confirm'");
