...
module.exports = {
	...
	async cmd_get_monitor() {
		...
		this.printSuggestedCommands({
			...
			"Delete monitor": `xy monitor delete ${monitor.id} --confirm`
		});
	},
	...
	async cmd_create_monitor() {
		...
		var params = this.prepareMonitorParams({
			data_type: 'float', suffix: '', groups: [], display: true,
			min_vert_scale: 0, icon: '', notes: '', data_match: '',
			delta: false, divide_by_delta: false, delta_min_value: false
		}, this.args, true);
	...
	async cmd_update_monitor() {
		...
		if (!Tools.numKeys(this.args)) return this.dieUsage('monitor update');
	...
	async cmd_test_monitor() {
		...
		var server = Tools.findObject(servers, { id: server_selector }) || this.findObjectFuzzy(servers, {
			title: server_selector, hostname: server_selector, ip: server_selector
		}, 1);
	...
	async cmd_delete_monitor() {
		var id = this.consumeMonitorID();
		if (this.args.confirm !== true) return this.die("Please confirm the monitor delete by adding '--confirm'");
