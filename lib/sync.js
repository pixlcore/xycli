...
module.exports = {
	...
	async runSyncCommand() {
		...
		// Both ordinary sync and setup completion commands report failures through
		// the shared warning and error collectors.
		this.warnings = [];
		this.errors = [];
		...
		var supported_lists = this.config.ui.list_list.map( list => list.id ).filter( id => !id.match(/^(secrets|users|roles)$/) );
		...
		// Bucket content needs the replacement API and data timestamps introduced
		// in xyOps 1.1.2. Check before any ordinary sync changes are applied.
		if ((sconfig.up && sconfig.up.includes('buckets')) || (sconfig.down && sconfig.down.includes('buckets')) ||
			(sconfig.delete && sconfig.delete.includes('buckets'))) {
			this.requireBucketSyncVersion();
		}
		...
		// Index XYPDF basenames by directory before attaching any neighbor files.
		// A source such as "Test-Event.json" is a prefix of both
		// "Test-Event-V2.json" and "Test-Event-V2-params.command.ps1".  Sorting
		// longest-first lets each external property file belong to the most specific
		// source basename instead of also being claimed by every shorter prefix.
		var source_stems_by_dir = new Map();
		var bucket_extra_dirs = [];
		files.filter( file => !!file.path.match(/\.json$/i) ).forEach( file => {
			var xypdf = null;
			try { xypdf = Tools.parseJSON( fs.readFileSync(file.path, 'utf8') ); }
			catch (err) { return; } // the normal scan below reports malformed JSON
			if (!xypdf || !Tools.isaHash(xypdf) || (xypdf.type !== 'xypdf')) return;
			if (Array.isArray(xypdf.items) && xypdf.items.length == 1 && xypdf.items[0].type == 'bucket') {
				bucket_extra_dirs.push(file.path.replace(/\.json$/i, ''));
			}
			
			var dir = Path.dirname(file.path);
			var stems = source_stems_by_dir.get(dir);
			if (!stems) {
				stems = [];
				source_stems_by_dir.set(dir, stems);
			}
			stems.push( Path.basename(file.path).replace(/\.\w+$/, '') );
		} );
		source_stems_by_dir.forEach( stems => {
		...
		files.filter( file => !!file.path.match(/\.json$/i) && !this.isSyncBucketExtraFile(file.path, bucket_extra_dirs) ).forEach( file => {
			...
			if (item.type.match(/^(secret|user|role)$/)) {
			...
			files.forEach( neighbor => {
				...
				if (!nstem.startsWith(stem + '-')) return; // no exact basename boundary
				
				// Multiple XYPDF sources may share a title prefix.  The longest matching
				// source stem owns this neighbor, so shorter sources must ignore it.
				var source_stems = source_stems_by_dir.get(dir) || [];
				var owner_stem = source_stems.find( source_stem => nstem.startsWith(source_stem + '-') );
				if (owner_stem && (owner_stem !== stem)) return;
				
				var prop_path = nstem.substring(stem.length + 1);
				if (!prop_path.match(/^[\w\.\-]+$/)) return; // not correct format
				
				var content = '';
				try {
					content = fs.readFileSync(neighbor.path, 'utf8');
				}
				catch (err) {
				...
				// A neighboring JSON file may be another freely-named sync source whose
				// stem happens to begin with this source's stem.  XYPDF files always stand
				// on their own and must never be interpreted as external property content.
				if (neighbor.path.match(/\.json$/i)) {
					try {
						var neighbor_json = Tools.parseJSON(content);
						if (neighbor_json && Tools.isaHash(neighbor_json) && (neighbor_json.type === 'xypdf')) return;
					}
					catch (err) {;} // leave non-JSON content for normal neighbor handling
				}
				
				// External files may only replace string properties that already exist in
				// the definition.  setSyncPropByPath() creates missing keys, so validate first.
				if (typeof(this.getSyncPropByPath(item.data, prop_path)) != 'string') {
					this.logSyncWarning("Neighbor file does not match an existing string property: " + neighbor.path + " (" + prop_path + ")");
					neighbor_error = true;
					return;
				}
				if (!this.setSyncPropByPath(item.data, prop_path, content)) {
					this.logSyncWarning("Invalid neighbor property path: " + neighbor.path + " (" + prop_path + ")");
					neighbor_error = true;
					return;
				}
			...
			// do not include item if neighbor loading or validation failed
		...
		for (const item of items) {
			...
			// Use the same highlighted, context-trimmed diff for every sync item.
			var diff = this.formatSyncDiff(changes);
			...
			if (direction == 'up') {
				...
				try {
					await this.callStandardAPI('update_' + item.type, item.data, { text: "Updating " + item.type + "...", throw: true, sync: true });
		...
		// Bucket metadata is handled above like every other XYPDF item. Its data
		// and binary files have their own storage records and reconciliation pass.
		if (sconfig.up && sconfig.up.includes('buckets') || sconfig.down && sconfig.down.includes('buckets')) {
			await this.syncBucketContents(items.filter( item => item.type == 'bucket' ), sconfig);
		}
		
		// Deletion is the destructive final phase, so only begin it after every
		// update and the sync-state write completed cleanly.  Earlier successful
		// updates are intentionally left in place and will compare equal on retry.
		if (sconfig.delete && (this.errors.length || this.warnings.length)) {
			this.logSyncWarning("Delete pass skipped because the sync encountered earlier warnings or errors.");
			return await this.completeSync();
		}
		...
		for (const list_name of sconfig.delete) {
			...
			for (const xy_item of list) {
				...
				try {
					await this.callStandardAPI('delete_' + type_def.id, { id: xy_item.id }, { text: "Deleting " + type_def.id + "...", throw: true, sync: true });
	...
	syncFinishCmd(cmd, opts) {
		// spawn command and log output
		opts = opts || {};
		var sconfig = this.sconfig || {};
		var cwd = opts.cwd || this.args.other[0]; // default to first normal sync base dir
		var cmd_timeout = opts.cmd_timeout || sconfig.cmd_timeout || 30;
	...
	downSyncItem(xy_item, item) {
		...
		if (item.neighbors) {
			for (var path in item.neighbors) {
				...
				var value = this.getSyncPropByPath( data, path ) ?? "";
				
				if (!this.setSyncPropByPath( data, path, "(External)" )) {
	...
	formatSyncDiff(changes) {
		// Render a diff code block for the CLI Markdown highlighter. Keep long
		// unchanged sections compact unless the user requested verbose output.
		var diff = "```diff\n";
	...
	async setupSync() {
		...
		// xy sync --setup events --new
		// xy sync --setup all --marketplace --stock --force
		if ((this.args.setup === 'all') || (this.args.setup[0] === 'all')) {
			this.args.setup = this.config.ui.list_list.map( list => list.id ).filter( id => !id.match(/^(secrets|users|roles)$/) );
		}
		...
		if (Tools.includesAny(this.args.setup, ['secrets', 'users', 'roles'])) {
			this.die("Sorry, the sync engine cannot process secrets, users, or roles.");
		}
		...
		if (this.args.setup.includes('buckets')) this.requireBucketSyncVersion();
		...
		// New-only setup is intentionally an additive operation.  It must never be
		// combined with the overwrite behavior of --force.
		var new_only = false;
		if (this.args.new !== undefined) {
			if ((this.args.new !== true) && (this.args.new !== false)) {
				this.die("Invalid --new value (use --new or --new false).");
			}
			new_only = this.args.new;
		}
		if (new_only && this.args.force) this.die("Sync setup --new cannot be combined with --force.");
		
		var default_ext = this.normalizeSyncDefaultExt(this.args.default_ext);
		var existing_keys = new_only ? this.scanSyncSourceIndex(['.']) : {};
		...
		this.printBoxList({
			...
			rows: [
				...
				['Default Ext', '.' + default_ext],
				new_only ? ['New Only', 'Yes'] : null,
		...
		var result = this.writeSyncSetupFiles(this.args.setup, {
			base_dir: '.',
			existing_keys: existing_keys,
			new_only: new_only,
			file_props: this.args.file_props || [],
			default_ext: default_ext,
			stock: !!this.args.stock,
			marketplace: !!this.args.marketplace,
			force: !!this.args.force,
			dry: !!this.dry
		});

		if (this.args.setup.includes('buckets')) {
			await this.setupBucketContents(result.bucket_plans, {
				force: !!this.args.force,
				dry: !!this.dry
			});
		}
		
		if (new_only && !result.written) println( "\n " + gray("No new definitions found.") );
		
		println( "\n " + this.color('theme').bold("Setup complete!") );
		
		// Setup writes into the current directory, not a positional base directory.
		// Only run an explicitly supplied down command after real files were written.
		if (!this.dry && result.written && this.args.down_cmd) {
			this.syncFinishCmd(this.args.down_cmd, {
				cwd: '.',
				cmd_timeout: this.args.cmd_timeout
			});
		}
	},
	
	normalizeSyncDefaultExt(value) {
		// Allow users to choose the fallback extension for extracted properties whose
		// language cannot be detected.  A leading dot is accepted for convenience,
		// but paths and compound values are rejected before any files are written.
		if (value === undefined) return 'txt';
		if (typeof(value) != 'string') {
			this.die("Invalid --default_ext value (use a filename extension such as ps1 or .ps1).");
		}
		
		var ext = value.trim().replace(/^\./, '');
		if (!ext.match(/^[A-Za-z0-9][A-Za-z0-9_]*$/)) {
			this.die("Invalid --default_ext value (use a filename extension such as ps1 or .ps1).");
		}
		return ext;
	},
	
	scanSyncSourceIndex(base_dirs) {
		// Build an identity index from an existing sync tree.  Filenames and folders
		// are deliberately ignored, because sync sources may be freely reorganized.
		var files = [];
		base_dirs.forEach( dir => {
			dir = Path.normalize(dir).replace(/[\\/]+$/, '') || '.';
			if (!fs.existsSync(dir)) this.die("Sync base dir does not exist: " + dir);
			files = files.concat( Tools.findFilesSync(dir, { stats: true }) );
		});
		...
		files.filter( file => !!file.path.match(/\.json$/i) ).forEach( file => {
			try {
				var xypdf = Tools.parseJSON( fs.readFileSync(file.path, 'utf8') );
				if (xypdf && xypdf.type == 'xypdf' && Array.isArray(xypdf.items) &&
					xypdf.items.length == 1 && xypdf.items[0].type == 'bucket') {
					bucket_extra_dirs.push(file.path.replace(/\.json$/i, ''));
				}
			}
			catch (err) {;} // report malformed source files during the real scan
		});
		files.filter( file => !!file.path.match(/\.json$/i) && !this.isSyncBucketExtraFile(file.path, bucket_extra_dirs) ).forEach( file => {
			var xypdf = null;
			try { xypdf = Tools.parseJSON( fs.readFileSync(file.path, 'utf8') ); }
			catch (err) { this.die("Failed to parse JSON file: " + file.path + ": " + err); }
			
			// Ordinary JSON files are unrelated and may coexist in the tree.
			if (!xypdf || !Tools.isaHash(xypdf) || (xypdf.type !== 'xypdf')) return;
			if (xypdf.version !== '1.0') this.die("File is not a compatible version of XYPDF: " + file.path);
			if (xypdf.xyops !== undefined && this.compareVersions(xypdf.xyops, this.xyopsVersion) > 0) {
				this.die("Item requires xyOps v" + xypdf.xyops + " or higher: " + file.path);
			}
			if (!Array.isArray(xypdf.items) || (xypdf.items.length !== 1)) {
				this.die("Sync source must contain exactly one item: " + file.path);
			}
			
			var item = xypdf.items[0];
			if (!item || !Tools.isaHash(item) || !item.type || (typeof(item.type) != 'string')) {
				this.die("Malformed item type in file: " + file.path);
			}
			if (item.type.match(/^(secret|user|role)$/)) {
				this.die("Unsupported item type: " + item.type + " in file: " + file.path);
			}
			if (!item.data || !Tools.isaHash(item.data) || !item.data.id || (typeof(item.data.id) != 'string')) {
				this.die("Malformed or missing item data or ID in file: " + file.path);
			}
			
			var type_def = this.config.ui.data_types[item.type];
			if (!type_def) this.die("Cannot locate internal type definition: " + item.type);
			var list = this[type_def.list];
			if (!list) this.die("Cannot locate internal list: " + type_def.list);
			if (!Tools.findObject(list, { id: item.data.id })) {
				this.die("Cannot find " + item.type + " in xyOps: " + (item.data.title || item.data.id) + " (" + item.data.id + ")");
			}
			
			var key = item.type + '-' + item.data.id;
			if (index[key]) {
				this.die("Duplicate source item found: " + file.path + " (already found in " + index[key].file + ")");
			}
			index[key] = { file: file.path, item: item };
		});
		
		return index;
	},
	
	isSyncBucketExtraFile(file, dirs) {
		// Bucket data and uploaded files are opaque to the XYPDF parser, even if
		// a user uploaded a .json file with non-JSON contents.
	...
	writeSyncSetupFiles(list_names, opts = {}) {
		// Write selected remote definitions using setup's suggested layout.  Keeping
		// this operation separate allows a future downsync to append newly discovered
		// remote definitions after it finishes updating existing local sources.
		...
		list_names.forEach( list_name => {
			if (list_name == 'events') {
				targets.push({ name: 'events', type: 'event', items: this.events.filter( event => event.type != 'workflow' ) });
				targets.push({ name: 'workflows', type: 'event', items: this.events.filter( event => event.type == 'workflow' ) });
				return;
			}
			
			var type_def = Tools.findObject( list_data_types, { list: list_name } );
			if (type_def) targets.push({ name: list_name, type: type_def.id, items: this[list_name] || [] });
		});
		
		var result = { written: 0, skipped: 0, bucket_plans: [] };
		var existing_keys = opts.existing_keys || {};
		var base_dir = opts.base_dir || '.';
		var default_ext = this.normalizeSyncDefaultExt(opts.default_ext);
		
		// Build and validate the complete write plan before creating any directories.
		// Events and workflows are grouped under their xyOps Category titles.  A
		// missing Category indicates inconsistent remote data, so setup must stop
		// without leaving behind a partially generated tree.
		targets.forEach( target => {
			var items = target.items.filter( item => {
				if (item.marketplace && !opts.marketplace) return false;
				if (item.stock && !opts.stock) return false;
				if (opts.new_only && existing_keys[target.type + '-' + item.id]) {
					result.skipped++;
					return false;
				}
				return true;
			...
			target.plans = items.map( source_item => {
				var dir = Path.join(base_dir, target.name);
				var display_dir = target.name + '/';
				
				if (target.type == 'event') {
					var category = Tools.findObject( this.categories, { id: source_item.category } );
					if (!category) {
						this.die("Cannot find Category for " + (source_item.type == 'workflow' ? 'workflow' : 'Event') + ": " + (source_item.title || source_item.id) + " (" + (source_item.category || 'missing ID') + ")");
					}
					
					var category_slug = this.getSyncSetupTitleSlug(category.title, 'Category', category.id);
					dir = Path.join(dir, category_slug);
					display_dir += category_slug + '/';
				}
				
				var title = this.getSyncSetupTitleSlug(source_item.title, (source_item.type == 'workflow' ? 'workflow' : target.type), source_item.id);
				return { source_item, title, dir, display_dir };
			});
			
			// Remote Event order is not guaranteed to keep Categories together. Group
			// plans explicitly so each printed folder heading owns every file shown
			// beneath it, while preserving the first-seen order of folders and items.
			var groups_by_dir = new Map();
			var plan_groups = [];
			target.plans.forEach( plan => {
				var group = groups_by_dir.get(plan.dir);
				if (!group) {
					group = [];
					groups_by_dir.set(plan.dir, group);
					plan_groups.push(group);
				}
				group.push(plan);
			});
			target.plans = [].concat(...plan_groups);
		});
		
		// All Categories are now known to be valid, so it is safe to start writing.
		targets.forEach( target => {
			var active_dir = '';
			target.plans.forEach( plan => {
				var dir = plan.dir;
				if (dir != active_dir) {
					if (!opts.dry) Tools.mkdirp.sync(dir);
					println( "\n " + cli.emoji('📂') + " " + bold.cyan(plan.display_dir) );
					active_dir = dir;
				}
			
				// Work on a deep copy so setup never changes the cached remote definition.
				var item = Tools.copyHash(plan.source_item, true);
				var extras = [];
				(opts.file_props || []).forEach( path => {
					var extra = this.getSyncPropByPath(item, path);
					if ((typeof(extra) == 'string') && extra.match(/\S/)) {
						extras.push({ path, content: extra });
						this.setSyncPropByPath( item, path, "(External)" );
					}
					
					// handle workflow nodes separately
					if (item.workflow && item.workflow.nodes && Array.isArray(item.workflow.nodes)) {
						item.workflow.nodes.forEach( node => {
							if (!node.data || !Tools.isaHash(node.data)) return; // sanity
							var extra = this.getSyncPropByPath(node.data, path);
							if ((typeof(extra) == 'string') && extra.match(/\S/)) {
								extras.push({ path: `workflow.${node.id}.${path}`, content: extra });
								this.setSyncPropByPath( node.data, path, "(External)" );
							}
						} );
					}
				});
				
				var data = this.getItemExportData(target.type, item);
				var title = plan.title;
				var filename = title + '.json';
				var file = Path.join(dir, filename);
				if (target.type == 'bucket') result.bucket_plans.push({ id: item.id, file: file });
				
				if (!opts.dry) {
					if (!opts.force && fs.existsSync(file)) this.die("File exists, add '--force' to overwrite all: " + file);
				...
				extras.forEach( extra => {
					...
					var ext = default_ext;
					if (command) {
						var detected_ext = this.getExtFromBinary(command);
						if (detected_ext != 'txt') ext = detected_ext;
					}
					else if (extra.content.trim().match(/^\{[\S\s]*\}$/) || extra.content.trim().match(/^\[[\S\s]*\]$/)) ext = 'json';
					
					var extra_filename = title + '-' + extra.path + '.' + ext;
					var extra_file = Path.join(dir, extra_filename);
					if (!opts.dry) {
						if (!opts.force && fs.existsSync(extra_file)) this.die("File exists, add '--force' to overwrite all: " + extra_file);
	...
	getSyncSetupTitleSlug(title, item_type, item_id) {
		// Use one filename-safe transformation for definitions and Category folders.
		// Reject unusable titles explicitly instead of silently producing `.json` or
		// collapsing an Event back into the top-level resource directory.
		if (typeof(title) != 'string') {
			this.die("Cannot create sync setup path for " + item_type + ": Missing title (" + item_id + ")");
		}
		
		var slug = title.replace(/\W+/g, '-').replace(/\-+$/, '').replace(/^\-+/, '');
		if (!slug) {
			this.die("Cannot create sync setup path for " + item_type + ": Title has no usable filename characters (" + item_id + ")");
		}
		return slug;
	},
	...
	getSyncPropByPath(data, path) {
		// special-case Tools.getPath for handling shorthand for named workflow nodes
		// e.g. `workflow.nabcd1234.params.foo` == `workflow.nodes[id='nabcd1234'].data.params.foo`
		if (path.match(/^workflow\.(\w+)\.(.+)$/) && data.workflow && data.workflow.nodes && Array.isArray(data.workflow.nodes)) {
			var node_id = RegExp.$1;
			var sub_path = RegExp.$2;
			if (node_id === 'nodes') return Tools.getPath(data, path);
			
			var node = Tools.findObject( data.workflow.nodes, { id: node_id } );
		...
		return Tools.getPath(data, path);
	},
	
	setSyncPropByPath(data, path, value) {
		// special-case Tools.setPath for handling shorthand for named workflow nodes
		// e.g. `workflow.nabcd1234.params.foo` == `workflow.nodes[id='nabcd1234'].data.params.foo`
		if (path.match(/^workflow\.(\w+)\.(.+)$/) && data.workflow && data.workflow.nodes && Array.isArray(data.workflow.nodes)) {
			var node_id = RegExp.$1;
			var sub_path = RegExp.$2;
			if (node_id === 'nodes') return Tools.setPath(data, path, value);
		...
		return Tools.setPath(data, path, value);
	}
