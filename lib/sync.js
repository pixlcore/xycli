...
module.exports = {
	...
	async setupSync() {
		...
		var default_ext = 'txt';
		if (this.args.default_ext !== undefined) {
			if (typeof(this.args.default_ext) != 'string') {
				this.die("Invalid --default_ext value (use a filename extension such as ps1 or .ps1).");
			}
			default_ext = this.args.default_ext.trim().replace(/^\./, '');
			if (!default_ext.match(/^[A-Za-z0-9][A-Za-z0-9_]*$/)) {
				this.die("Invalid --default_ext value (use a filename extension such as ps1 or .ps1).");
			}
		}
