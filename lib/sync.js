...
module.exports = {
	...
	async cmd_sync() {
		...
		// xy sync --up all --dry
		// xy sync --up "events,plugins" --down events --delete plugins
		...
		var files = [];
		if (sconfig.base_dirs) this.args.other = sconfig.base_dirs;
		if (!this.args.other || !this.args.other.length) this.args.other = ['.'];
		...
		files.filter( file => !!file.path.match(/\.json$/i) ).forEach( file => {
			...
			// if item is upsync only, add sync flag
			var upsync = sconfig.up && sconfig.up.includes(type_def.list);
			var downsync = sconfig.down && sconfig.down.includes(type_def.list);
			
			// upsync only: add sync flag so xyops shows edit warning in the UI
			if (upsync && !downsync) item.data.sync = true;
			
			// bidirectional sync: xy item has an old sync flag, so set ours to false to deliberately overwrite it
			if (upsync && downsync && xy_item.sync) item.data.sync = false;
