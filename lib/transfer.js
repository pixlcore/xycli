...
const TYPES = {
	...
	web_hook: { command: 'webhook', list: 'web_hooks', api: 'WebHook' }
};
...
module.exports = {
	...
	async cmd_export_object(command) {
		...
		var type = Object.keys(TYPES).find( type => TYPES[type].command == command );
	...
	async cmd_import() {
		...
		if (this.dry || this.args.confirm !== true) {
			...
			if (!this.format.match(/json/)) {
				println("\n Preview only. Review the data above, then use --confirm to apply the import.");
				this.printSuggestedCommands({ "Apply this import": "xy import " + this.quoteTransferFilename(filename) + " --confirm" });
			}
