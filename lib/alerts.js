...
module.exports = {
	...
	async cmd_get_alert() {
		...
		// Invocation details include paginated related records; definitions do not.
		if (resolved.type == 'invocation') this.prepSearchArgs();
	...
	async cmd_update_alert_definition() {
		...
		if (!await this.confirmSyncUpdate('alert', alert.id, 'alert')) return;
		if (!Tools.numKeys(this.args)) return this.die("No updates specified for Alert Definition.");
	...
	async cmd_delete_alert() {
		...
		if (resolved.type == 'definition') {
			this.printMutationSummary({
				...
				rows: [
					[ 'Alert ID', gray(resolved.alert.id) ],
					[ 'Title', this.color('theme').bold(resolved.alert.title) ]
				]
			});
		}
		else {
			this.printMutationSummary({
				title: 'Delete Alert Invocation',
				rows: [
					[ 'Invocation ID', gray(resolved.alert.id) ],
					[ 'Message', this.color('theme').bold(resolved.alert.message || '(None)') ]
				]
			});
		}
		...
		if (this.args.confirm !== true) {
			if (resolved.type == 'definition') {
				this.toast('⚠️', 'orange', "Deleting Alert Definition #" + id + " also clears its live state and starts background deletion of all its Alert Invocations. Add '--confirm' to continue.");
				return;
			}
			this.toast('⚠️', 'orange', "Deleting Alert Invocation #" + id + " is permanent. Add '--confirm' to continue.");
			return;
		}
	...
	async printAlertInvocation(alert) {
		...
		this.printSuggestedCommands({
			...
			"Delete this invocation": `xy alert delete ${alert.id}`
		});
	},
