...
module.exports = {
	...
	jsonOutput(data) {
		...
		if (this.format && this.format.match(/jsonc/)) {
			...
			process.stdout.write( "\n" + gray( this.highlight(disp_value) ) );
		}
	},
	...
	printParamValues(opts) {
		...
		opts.fields.forEach( field => {
			...
			switch (field.type) {
				...
				case 'text':
					if ((field.variant == 'password') && !cli.args.verbose) disp_value = yellow('(Shown in verbose mode)');
				...
				case 'code':
				case 'json':
					disp_value = cli.args.verbose ? green('(See below)') : yellow('(Shown in verbose mode)');
				break;
		...
		if (cli.args.verbose) {
			opts.fields.filter( field => !!field.type.match(/^(textarea|code|json)$/) ).forEach( field => {
				...
				println( "\n " + this.color('theme').bold( field.title.toUpperCase() + ':' ) );
