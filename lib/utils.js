...
// marked-terminal hardcodes an asterisk as its unordered-list marker.  Wrap
// only the completed list renderer so its numbering, nesting, and reflow logic
// stay intact, while presenting a friendlier typographic bullet to the user.
const renderTerminalList = terminalRenderer.renderer.list;
terminalRenderer.renderer.list = function(...args) {
	return renderTerminalList.apply(this, args).replace(/^([ \t]*)\* /gm, '$1• ');
};
