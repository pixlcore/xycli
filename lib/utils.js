...
const STANDARD_API_PROPERTY_RULES = {
	...
	'^(create_api_key|update_api_key)$': [
		'id', 'key', 'mask', 'active', 'privileges', 'roles', 'title', 'icon', 
	...
	'^(create_group|update_group)$': [
		'id', 'title', 'hostname_match', 'icon', 'alert_actions', 'notes',
		'max_jobs_per_server'
	],
...
module.exports = {
	...
	async confirmSyncUpdate(type, id, thing) {
		// Up-only filesystem syncs are authoritative for their managed objects.
		// The composite type-ID key matches the state written by the sync engine.
		await this.getMultiple();
		
		if (('confirm' in this.args) && (typeof(this.args.confirm) != 'boolean')) {
			return this.die("Update --confirm must be true or false.");
		}
		
		var confirmed = (this.args.confirm === true);
		delete this.args.confirm;
		
		var state_key = type + '-' + id;
		if (!this.state || !this.state.sync || !this.state.sync[state_key]) return true;
		if (confirmed) return true;
		
		this.toast('⚠️', 'yellow', "This " + thing + " is under remote management, and should not be edited here.  Add '--confirm' to continue.");
		return false;
	},
	...
	getLangFromBinary(bin) {
		...
		// Normalize both path styles because Plugin definitions may come from a
		// different operating system than the CLI displaying them.  Remove command
		// arguments before taking the basename, including quoted paths with spaces.
		...
		cmd = Path.posix.basename(cmd);
		
		// Windows executables conventionally include .exe.  Version suffixes are
		// also common for interpreters such as python3.12 and node24.
	...
	async cacheConfig() {
		...
		// merge, but do not clobber keys already in local config
	...
	async callStandardAPI(name, req, opts = {}) {
		...
		if (!opts.sync) this.validateStandardAPIRequest(name, req);
		delete opts.sync;
		...
		if (this.dry || this.verbose) {
			println( "\n " + this.color('theme').bold( this.dry ? `API REQUEST PREVIEW: ${name}` : `API REQUEST: ${name}` ) );
		...
		if (this.verbose) {
			println( "\n " + this.color('theme').bold("API RESPONSE: " + name) );
	...
	compareVersions(left, right) {
		// Compare semantic version components without adding a dependency. Build
		// metadata does not affect precedence; prereleases sort below releases.
		var parse = value => {
			var match = typeof(value) == 'string' && value.match(/^(\d+)\.(\d+)\.(\d+)(?:-([\w.-]+))?(?:\+[\w.-]+)?$/);
			if (!match) this.die("Invalid version: " + value);
			return match;
		};
