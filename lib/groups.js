...
module.exports = {
	...
	async cmd_get_group() {
		...
		this.printSuggestedCommands({
			...
			'Delete this group': 'xy group delete ' + group.id + ' --confirm'
		});
	},
	...
	printGroupQuickmonCharts(group, servers, quickmon, mode) {
		var ids = new Set(servers.filter( server => !server.offline ).map( server => server.id ));
		...
		println('\n ' + this.color('theme').bold('GROUP QUICK LOOK - LAST MINUTE'));
	...
	printGroupMonitorCharts(group, snapshots, mode, view, range) {
		var monitors = this.getVisibleGroupMonitors(group);
		var heading = view == 'history' ? 'GROUP MONITORS - ' + range.title.toUpperCase() : 'GROUP MONITORS - LAST HOUR';
