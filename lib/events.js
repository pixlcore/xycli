...
module.exports = {
	...
	async cmd_get_events() {
		...
		Tools.sortBy( this.categories, 'sort_order', { type: 'number', dir: 1 } ).forEach( category => {
			...
			var rows = [
                ['Event ID', 'Title', 'Plugin', 'Targets', 'Triggers', 'Status', 'Modified']
            ];
            cat_events.forEach( event => {
                rows.push([ 
					this.getNiceEventID(event.id),
					this.getNiceEvent(event.id),
	...
	async cmd_get_event() {
		...
		// notes
		if (event.notes) {
			this.printUserNotes('EVENT NOTES', event.notes);
		}
	...
	async cmd_update_event() {
		...
		delete this.args.other;
		if (!await this.confirmSyncUpdate('event', event.id, event.workflow ? 'workflow' : 'event')) return;
		if (!Tools.firstKey(this.args)) return this.die("No updates specified for event.");
		
		this.printUpdateData(this.args);
		
		var params = this.prepareEventUpdateParams(event, this.args);
		params.id = event.id;
		
		var data = await this.callStandardAPI('updateEvent', params, {
			text: 'Updating event...'
		});
		if (this.dry) return;
	...
	getNiceEventID(event) {
		if (!event) return gray('(None)');
		if (typeof(event) == 'string') {
			event = Tools.findObject( this.events, { id: event } ) || { id: event, title: event };
		}
		
		if (!event.enabled) return gray(event.id);
		
		var category = Tools.findObject( this.categories, { id: event.category } );
		if (category && category.color && (category.color != 'plain')) {
			return this.color(category.color).bold(event.id);
		}
		
		return this.color('theme').bold(event.id);
	},
	
	prepareEventUpdateParams(event, input) {
		// Build a sparse API request so properties introduced by a newer xyOps server
		// are never echoed back through an older CLI.  For dotted edits, clone only
		// the affected saved container because xyOps shallow-merges top-level fields.
		var params = {};
		var dotted = {};
		
		Object.keys(input).filter( key => !key.includes('.') ).forEach( key => {
			params[key] = input[key];
		});
		Object.keys(input).filter( key => key.includes('.') ).forEach( key => {
			var root = key.split('.')[0];
			if (!Object.prototype.hasOwnProperty.call(params, root) && Object.prototype.hasOwnProperty.call(event, root)) {
				params[root] = Tools.copyHash({ value: event[root] }, true).value;
			}
			dotted[key] = input[key];
		});
		this.mergeDotArgs(params, dotted);
		
		// Singular append options and trigger shortcuts need the saved array so they
		// preserve existing entries.  Whole-array options above remain replacements.
		var append_fields = {
			trigger: 'triggers',
			action: 'actions',
			limit: 'limits',
		...
		Object.keys(append_fields).forEach( singular => {
			var plural = append_fields[singular];
			if ((singular in params) && !(plural in params)) {
				params[plural] = Tools.copyHash({ value: event[plural] || [] }, true).value;
			}
		});
		
		var trigger_shortcuts = ['cron', 'interval', 'single', 'magic', 'seconds', 'delay', 'invisible', 'ephemeral', 'catchup'];
		if (trigger_shortcuts.some( key => key in params ) && !('triggers' in params)) {
			params.triggers = Tools.copyHash({ value: event.triggers || [] }, true).value;
		}
		
		this.processEventUpdates(params, event);
		return params;
	},
	
	processEventUpdates(event, saved_event) {
		// convert convenience param syntax into event object
		
		// Creation retains the original empty-array defaults.  Sparse updates only
		// carry arrays which are explicitly replaced, edited or appended above.
		if (!saved_event) {
		...
		if (event.cron) {
			var trigger = this.parseCrontab(event.cron, event.title || (saved_event && saved_event.title));
