...
module.exports = {
	...
	async cmd_events() {
		// router, e.g. `xy events list`
		var cmd = this.args.other.shift() || 'list';
		switch (cmd) {
			case 'list': await this.cmd_get_events(); break;
			case 'get': await this.cmd_get_event(); break;
			case 'run': await this.cmd_run_event(); break; // TODO: this
			...
			default: 
				// assume command is search criteria
	...
	async cmd_get_events() {
		...
		// xy events list foobar == search for title:foobar
		...
		this.printSuggestedCommands({
			...
			"Get event details": "xy events get ID_OR_TITLE"
		});
	},
	...
	async cmd_get_event() {
		...
		// suggested commands
		this.printSuggestedCommands({
			"Upcoming jobs": `xy events get ${event.id} --upcoming`,
			"Completed jobs": `xy events get ${event.id} --completed`,
			"Run job manually": `xy run ${event.id}`,
			"Update event": `xy events update ${event.id} [--KEY VALUE, ...]`,
			"Delete event": `xy events delete ${event.id}`
		});
	},
