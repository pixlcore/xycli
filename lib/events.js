...
module.exports = {
	...
	async cmd_run_event() {
		...
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Running event...') });
	...
	async cmd_create_event() {
		...
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Creating event...') });
		var { err, data } = await this.api.createEvent(params);
		if (err) this.die(err);
		cli.progress.end();
	...
	async cmd_update_event() {
		...
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Updating event...') });
		var { err, data } = await this.api.updateEvent(event);
		if (err) this.die(err);
		cli.progress.end();
	...
	async cmd_delete_event() {
		...
		if (!params.confirm) this.die("Please confirm the delete by adding '--confirm'");
		delete params.confirm;
		
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Deleting event...') });
		var { err, data } = await this.api.deleteEvent(params);
		if (err) this.die(err);
		cli.progress.end();
