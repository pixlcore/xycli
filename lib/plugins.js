...
module.exports = {
	...
	async cmd_get_plugin() {
		...
		// Always advertise the script section, but keep the potentially large source
		// behind verbose mode. Prefer the executable's language, then the script's
		// shebang, and finally let cli-highlight perform its slower auto-detection.
		if (script) {
			...
			if (!this.verbose) {
				...
				println( this.highlight(source_lines, highlight_opts) );
			}
		}
		...
		this.printSuggestedCommands({
			...
			"Delete Plugin": `xy plugin delete ${plugin.id} --confirm`
		});
	},
	...
	async cmd_update_plugin() {
		...
		if (!Tools.numKeys(this.args)) return this.dieUsage('plugin update');
	...
	async cmd_delete_plugin() {
		var id = this.consumePluginID();
		if (this.args.confirm !== true) return this.die("Please confirm the Plugin delete by adding '--confirm'");
