...
module.exports = {
	...
	async cmd_get_category() {
		...
		this.printBoxList({
			...
			rows: [
				...
				[ 'Notes', category.notes || gray('(None)') ],
	...
	async cmd_update_category() {
		...
		if (!Tools.numKeys(this.args)) return this.dieUsage('category update');
	...
	async cmd_delete_category() {
		...
		var id = this.consumeCategoryID();
		if (this.args.confirm !== true) return this.die("Please confirm the category delete by adding '--confirm'");
