...
module.exports = {
	...
	async cmd_get_bucket() {
		...
		this.printSuggestedCommands({
			...
			"Delete a file": result.files.length ? `xy bucket file delete ${bucket.id} ${result.files[0].filename}` : '',
			"Empty bucket contents": `xy bucket empty ${bucket.id} --data --files`,
			"Delete bucket": `xy bucket delete ${bucket.id}`
		});
	},
