...
module.exports = {
	...
	async cmd_get_bucket() {
		...
		this.printBoxList({
			...
			rows: [
				...
				[ 'Notes', result.bucket.notes || gray('(None)') ],
	...
	async cmd_update_bucket() {
		...
		if (!Tools.numKeys(this.args)) return this.dieUsage('bucket update');
	...
	async cmd_delete_bucket() {
		...
		delete this.args.id;
		delete this.args.other;
		if (!this.args.confirm) return this.die("Please confirm the bucket delete by adding '--confirm'");
		delete this.args.confirm;
		if (Tools.numKeys(this.args)) this.assertBucketFields(this.args, []);
