...
module.exports = {
	...
	async cmd_update_channel() {
		...
		if (!Tools.numKeys(this.args)) return this.dieUsage('channel update');
	...
	async cmd_delete_channel() {
		...
		var id = this.consumeChannelID();
		if (this.args.confirm !== true) return this.die("Please confirm the channel delete by adding '--confirm'");
