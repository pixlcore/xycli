...
module.exports = {
	...
	async cmd_get_channel() {
		...
		if (channel.notes) {
			this.printUserNotes('CHANNEL NOTES', channel.notes);
		}
		...
		this.printSuggestedCommands({
			...
			"Delete channel": `xy channel delete ${channel.id}`
		});
	},
	...
	async cmd_update_channel() {
		...
		if (!await this.confirmSyncUpdate('channel', channel.id, 'notification channel')) return;
	...
	async cmd_delete_channel() {
		...
		if (this.args.confirm !== true) {
			this.toast('⚠️', 'orange', "Please confirm the channel delete by adding '--confirm'.");
			return;
		}
		...
		var data = await this.callStandardAPI('deleteChannel', { id: id }, { text: 'Deleting channel...' });
		if (this.dry) return;
		if (this.format.match(/json/)) return this.jsonOutput(data);
		this.toast('✅', 'green', "Successfully deleted channel: #" + id);
	},
	
	async fetchChannel(id) {
		// Fetch authoritative metadata through the single-channel endpoint.
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Loading channel...') });
