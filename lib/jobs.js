	...
	async cmd_stream_job() {
		...
		this.printBoxList({
			...
			rows: [
				...
				(job.type != 'workflow') ? [ 'Server', this.getNiceServer(job.server) + ` ${gray('(' + job.server + ')')}` ] : null,
		...
		(job.activity || []).forEach( row => {
			...
			cli.println( green('[' + this.getNiceServer(String(row.server).replace(/^m:/, '')) + '] ') + gray( String(row.msg).trim() ) );
		} );
		...
		var { err } = await this.api.streamJob({ id: this.args.id, output: true, meta: true }, data => {
			...
			if ((data.type === 'output') && ('text' in data) && (typeof(data.text) == 'string')) {
				...
				cli.println( green('[' + this.getNiceServer(String(data.row.server).replace(/^m:/, '')) + '] ') + gray( String(data.row.msg).trim() ) );
			}
			else {
	...
	async cmd_get_job() {
		...
		this.printBoxList({
			...
			rows: [
				...
				job.server ? [ 'Server', this.getNiceServer(job.server) + ` ${gray('(' + job.server + ')')}` ] : null,
