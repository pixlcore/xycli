...
module.exports = {
	...
	async cmd_run_job_again() {
		...
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Running job again...') });
		var { err, data } = await this.api.runEvent(new_job);
		if (err) this.die(err);
		cli.progress.end();
	...
	async cmd_resume_job() {
		...
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Resuming job...') });
		var { err, data } = await this.api.resumeJob(req, opts);
		if (err) this.die(err);
		cli.progress.end();
	...
	async cmd_abort_job() {
		...
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Aborting job...') });
		var { err, data } = await this.api.abortJob(req, opts);
		if (err) this.die(err);
		cli.progress.end();
	...
	async cmd_delete_job() {
		...
		cli.progress.start({ amount: 1, pct: false, text: gray('→ Deleting job...') });
		var { err, data } = await this.api.deleteJob(req, opts);
		if (err) this.die(err);
		cli.progress.end();
	...
	printJobFiles(job) {
		...
		var rows = files.map( file => {
			...
			return [
				...
				nice_source,
				this.getNiceJob(file.job || job.id),
				file.server ? this.getNiceServer(file.server) : gray('(None)'),
				gray(file.path || '(Unknown)')
			];
		} );
		
		this.printBoxTable({
			title: 'Job Files',
			header: [ 'Filename', 'Size', 'Modified', 'Source', 'Job', 'Server', 'Storage Path' ],
			rows: rows
		});
