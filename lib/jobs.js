...
module.exports = {
	...
	async cmd_jobs() {
		...
		switch (cmd) {
			...
			case 'log': await this.cmd_get_job_log(); break; // TODO: this
	...
	async cmd_job() {
		// alias for `jobs get`
		if (this.args.other && (this.args.other.length == 2) && (this.args.other[0] == "log")) {
			this.args.other.shift();
			return await cmd_get_job_log();
		}
	...
	async cmd_get_job() {
		...
		if (job.perf) {
			...
			println( this.highlight(disp_value) );
		}
	...
	async cmd_get_job_log() {
		...
		var { err } = await this.api.getJobs({
			id: this.args.id,
			download: this.args.download || process.stdout
		});
		if (err) this.die(err);
	...
	printMetaLog(job) {
		// print job meta log in verbose mode
		println( "\n " + this.color('theme').bold( "JOB META LOG" ) );
		if (!this.verbose) {
			println( " " + gray("(Shown in verbose mode)") );
			return;
		}
