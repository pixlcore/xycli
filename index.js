...
const app = {
	...
	async run() {
		...
		if (!this['cmd_' + cmd]) {
			...
			else this.die("Unknown command: " + cmd, "Available Commands: help, config, dashboard, system, upcoming, alerts, alert, buckets, bucket, categories, category, channels, channel, events, event, run, jobs, job, keys, key, log, monitors, monitor, plugins, plugin, servers, server, secrets, secret, tags, tag, tickets, ticket, hooks, hook, marketplace, import, api, repl\n\n");
		}
